 ...
 * valid.
 *
 * @param chip8 The structure representing the state of the chip8 machine.
 */
 ...
 *
 * @param chip8 The pointer to the struct to actually initialize.
 */
 ...
 *
 * @param chip8 The structure to manipulate when steping through the program.
 */
void step(struct chip8 *chip8)
{
    // Fetch Instruction
    uint16_t instr = (chip8->memory[chip8->reg_PC] << 8)
                        | chip8->memory[chip8->reg_PC+1];
    ...
    // For use in arithmetic/bitwise instructions
    int result;
        ...
        case 23:
            ...
            for (int j = 0; j < nibble; j++) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    chip8->regs[0xF] = chip8->regs[0xF]
                                        | (chip8->screen[pos] & pix);
